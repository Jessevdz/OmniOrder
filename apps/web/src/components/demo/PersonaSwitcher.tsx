...
export const PersonaSwitcher: React.FC<PersonaSwitcherProps> = ({ currentPreset, onPresetChange }) => {
    ...
    // Default to minimized (though this won't matter on mobile anymore due to the check below)
    const [isMinimized, setIsMinimized] = useState(() => window.innerWidth < 768);

    // Check for mobile to adjust layout logic
    ...
    // --- MOBILE HIDE LOGIC ---
    // Since mobile navigation is handled by the static top bar, 
    // we return null here to remove this floating component entirely on small screens.
    if (isMobile) {
        return null;
    }
    ...
    if (isMinimized) {
        return (
            <button
                ...
                // Desktop: Fixed Bottom-Left. Shape: Circle.
                className={`
                    ...
                    bottom-8 left-8 w-14 h-14 justify-center rounded-full hover:scale-105
                `}
                title="Open Demo Controls"
            >
                <SlidersHorizontal size={20} />

                {/* Attention Grabber: Pulsing Dot */}
    ...
    const containerPosition = 'bottom-8 left-8 items-start';
    ...
    return (
        ...
        >
            ...
            {/* Desktop: Show ABOVE (order-first) */}
            {showThemes && (
                <div className="bg-white/90 backdrop-blur-md border border-gray-200 p-2 rounded-2xl shadow-2xl w-64 order-first mb-1 origin-bottom-left">
            ...
            <div className="bg-neutral-900/95 backdrop-blur-md border border-neutral-700 p-1.5 rounded-full shadow-2xl flex items-center gap-1">
                ...
                {/* Vertical Divider */}
                <div className="w-px h-6 bg-neutral-700 mx-1 shrink-0 block"></div>
